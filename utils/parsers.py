...
class Parsers:
    ...
    def add_argument(self, argument_name: str, is_required: bool = False, get_value: bool = False,
                     value_placeholder: str = "argument") -> None:
        """
        ...
        for argument in self.arguments:
            ...
            if (argument["name"] not in self.args):
                ...
                if argument["required"]:
                    await self.ctx.send(embed=Embed(title="Missing required argument",
                                                    description="This command is missing a `{}`.\nUsage: ```{}```".format(
                                                        argument["name"], self.usage)).set_footer(
                        text="P.S: <arg> is required and [arg] is optional. Do NOT literally type the [] or <>."))
                    return _res_dict
                continue
    ...
    def is_mentionable(name: str):
        ...
        for char in name:
            if char in Parsers.ALPHABET: return True
        return False
    ...
    def parse_image(ctx, args, default_to_png=True, size=1024, member_only=False, cdn_only=False):
        ...
        if (len(ctx.message.attachments) > 0) and (not member_only):
            res = Parsers.__check_url(ctx.message.attachments[0].url, cdn_only)
            if res:
        ...
        if ((url.startswith("http")) and ("://" in url)) and (not member_only):
            if (not member_only) and Parsers.__check_url(url, cdn_only):
                return url
        ...
        user = Parsers.parse_user(ctx, args)
    ...
    def parse_user(ctx, args, allownoargs=True):
        ...
        member = list(
            filter(lambda x: (x.display_name.lower().startswith(user_name)) or (user_name in x.display_name.lower()),
                   ctx.guild.members))
    ...
    def split_content_to_two(args: tuple):
        ...
        for char in Parsers.SPLIT_CHARACTERS:
            if char in args_as_a_string: return args_as_a_string.split(char)[0], char.join(
                args_as_a_string.split(char)[1:])
    ...
    def html_to_markdown(text: str) -> str:
        ...
        result = text.lower()
        for key in Parsers.HTML_DICT.keys():
            result = result.replace(key, Parsers.HTML_DICT[key])
        return result