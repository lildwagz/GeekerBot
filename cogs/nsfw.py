...
class nsfwcog(commands.Cog, name="nsfwcog"):
    ...
    async def displayHelp(self):
        ...
        helpEmbed.add_field(name=".pornhub home",
                            value='Lists some of the "Hottest" and "Most Viewed" videos from the front page of PornHub',
                            inline=False)
        helpEmbed.add_field(name=".pornhub search <query> [page] [minRating]",
                            value="Allows you to search PornHub for videos. Simple as that!", inline=False)
        helpEmbed.add_field(name=".pornhub category <categoryName> [page] [minRating]",
                            value="Allows you to browse PornHub by category", inline=False)
        helpEmbed.add_field(name=".pornhub category list",
                            value="Sends you a comprehensive list of all available categories you can use with *.pornhub category*",
                            inline=False)
        helpEmbed.add_field(name=".pornhub hottest [page] [minRating]",
                            value='Allows you to browse the "Hottest" videos on PornHub', inline=False)
        helpEmbed.add_field(name=".pornhub mostviewed [page] [minRating]",
                            value='Allows you to browse the "Most Viewed" videos on PornHub', inline=False)
        helpEmbed.add_field(name=".pornhub toprated [page] [minRating]",
                            value='Allows you to browse the "Top Rated" videos on PornHub', inline=False)
    ...
    async def printVids(self, vids, context, query, page, rating):
        if len(vids) <= 0:
            ...
            for i in range(len(vids)):
                ...
                pornStats = '\t:clock2: ' + vids[i][2] + '\t:eyes: ' + vids[i][3] + '\t' + goodBad + ' ' + vids[i][
                    4] + '%'
            ...
            self.tasks.append(
                asyncio.ensure_future(self.bot.wait_for_message(author=context.message.author, check=check,
            ...
            vidStats = '\t:clock2: ' + selectedVid[2] + '\t:eyes: ' + selectedVid[3] + '\t' + goodBad + ' ' + \
                       selectedVid[4] + '%'
    ...
    async def search(self, context, query: str = "help", page: int = 1, rating: int = 0):
        if query.lower() == "help":
            helpEmbed = discord.Embed(title="*.pornhub search <query> [page] [minRating]*",
        ...
        parsedQuery = parse.quote_plus(query)
    ...
    @pornhub.command(pass_context=True)
    async def category(self, ctx, categoryName: str = "help", page: int = 1, rating: int = 0):
        if categoryName.lower() == "help":
            helpEmbed = discord.Embed(title="*.pornhub category <categoryName> [page] [minRating]*",
                                      colour=discord.Colour(0xFF9900))
    ...
    async def home(self, context):
        ...
        goodBad = ':thumbsup:' if int(selectedVid[4]) >= 50 else ':thumbsdown:'
        vidUrl = 'https://www.pornhub.com/view_video.php?viewkey=' + selectedVid[0]
        vidStats = '\t:clock2: ' + selectedVid[2] + '\t:eyes: ' + selectedVid[3] + '\t' + goodBad + ' ' + selectedVid[
            4] + '%'
        catString = ", ".join(selectedVid[5])
        tagString = ", ".join(selectedVid[6])
