...
class OnMessageSpam(commands.Cog, name="on message Spam"):
        ...
        if antiSpam:
            ...
            try:
                ...
                if len(list(filter(lambda m: check(m), self.bot.cached_messages))) >= 8 and len(
                        list(filter(lambda m: check(m), self.bot.cached_messages))) < 14:
                ...
                # Logs
                # try:
                #     embed = discord.Embed(title=f"**{message.author} has been kicked.**",
                #                           description=f"**Reason :** He spammed in {message.channel}.\n\n**__User informations :__**\n\n**Name :** {message.author}\n**Id :** {message.author.id}",
                #                           color=0xff0000)
