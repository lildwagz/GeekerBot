...
class animotes(commands.Cog,name="animotes"):
...
#         except Exception:
#             logging.exception(
#                 'The bot does not have the ability to remove this message. Make sure the bot has manage message permissions.')
#
...
#                 if mimetype:
#                     os.rename(src='{0}/{1}'.format(temporary_dir, filename),
#                               dst='{0}/{1}{2}'.format(temporary_dir, filename, mimetype))
...
#         self.conn.cursor().execute('SELECT * FROM animotes WHERE user_id=?',
        ...
        for guild in self.bot.guilds:
            ...
            if emoji:
                ...
                for emoji in guild.emojis:
                    if emoji.animated:
                        message.add_line(f'**{emoji.id} {emoji.name}**: {emoji}')
                message.add_line('')
        if not message:
