...
class meme(commands.Cog):
    ...
    async def clint(self, ctx, *args):
        url = ctx.bot.Parser.parse_image(ctx, args)
        await ctx.trigger_typing()
        return await ctx.bot.util.send_image_attachment(ctx,
                                                        "https://useless-api.vierofernando.repl.co/clint?image=" + url,
                                                        uselessapi=True)

    @commands.command("ltt,lienus")
    @cooldown(7)
    async def linus(self, ctx, *args):
        url = ctx.bot.Parser.parse_image(ctx, args)
        await ctx.trigger_typing()
        return await ctx.bot.util.send_image_attachment(ctx,
                                                        "https://useless-api.vierofernando.repl.co/linus?image=" + url,
                                                        uselessapi=True)
    ...
    async def folder(self, ctx, *args):
        ...
        await ctx.trigger_typing()
        return await ctx.bot.util.send_image_attachment(ctx,
                                                        "https://useless-api.vierofernando.repl.co/folder?image=" + url,
                                                        uselessapi=True)

    @commands.command('scoobydoo,reveal,revealed,expose,exposed,scooby-doo')
    @cooldown(2)
    ...
    async def password(self, ctx, *args):
        ...
        if param is None: return await ctx.bot.util.send_error_message(ctx,
                                                                       "Please send two parameters, either split by a space, a comma, or a semicolon.")
        await ctx.trigger_typing()
        text1, text2 = param
        i = await ctx.bot.canvas.password(text1, text2)
        await ctx.send(file=discord.File(i, 'password.png'))

    @commands.command('programmerhumor,programmermeme,programming,programmer')
    @cooldown(2)
    async def programmingmeme(self, ctx):
        ...
        return await ctx.send(
            embed=discord.Embed(title='Programmer meme', color=ctx.guild.me.roles[::-1][0].color).set_image(
                url=data['url']))

    @commands.command('shred,burn,spongebobpaper,paper,spongepaper,sponge-paper,spongebob-paper,spongebob')
    @cooldown(2)
    ...
    @commands.command('ihavefailedyou,fail')
    @cooldown(2)
    ...
    @commands.command('gruplan,plan')
    @cooldown(4)
    async def gru(self, ctx, *args):
        if '; ' not in ' '.join(args): return await ctx.bot.util.send_error_message(ctx,
    ...
    @commands.command('crazy-frog,crazyfrogdance,dance,crazy-dance,kiddance,kid-dance')
    @cooldown(7)
    ...
    @commands.command('destroycomputer,smash')
    @cooldown(5)
    ...
    @commands.command('blowup,blow,death-star')
    @cooldown(10)
    ...
    @commands.command('evol,trashevol,evoltrash,evolutiontrash')
    @cooldown(5)
    ...
    @commands.command('lookatthisgraph')
    @cooldown(5)
    ...
    #         url = 'https://api.alexflipnote.dev/challenge?text=' + txt
    #     elif command_name == "call":
    #         url = 'https://api.alexflipnote.dev/calling?text=' + txt
    #     else:
    #         url = 'https://api.alexflipnote.dev/achievement?text=' + txt
    #     return await ctx.bot.util.send_image_attachment(ctx, url, alexflipnote=True)

    @commands.command('dym')
    @cooldown(2)
    async def didyoumean(self, ctx, *args):
        params = ctx.bot.Parser.split_content_to_two(args)
        if params is None: return await ctx.bot.util.send_error_message(ctx,
                                                                        "Please send two parameters, either split by a space, a comma, or a semicolon.")
        txt1, txt2 = params
    ...
    async def drake(self, ctx, *args):
        ...
        if params is None: return await ctx.bot.util.send_error_message(ctx,
                                                                        "Please send two parameters, either split by a space, a comma, or a semicolon.")
        txt1, txt2 = params
        url = "https://api.alexflipnote.dev/drake?top=" + ctx.bot.util.encode_uri(
            txt1[0:50]) + "&bottom=" + ctx.bot.util.encode_uri(txt2[0:50])
        return await ctx.bot.util.send_image_attachment(ctx, url, alexflipnote=True)
    ...
    async def what(self, ctx, *args):
        ...
        return await ctx.bot.util.send_image_attachment(ctx, "https://api.alexflipnote.dev/what?image=" + image,
                                                        alexflipnote=True)
    ...
    @commands.command('communism,ussr,soviet,cykablyat,cyka-blyat,blyat')
    @cooldown(5)
    ...
    @commands.command('mywaifu,wf,waifuinsult,insultwaifu,waifu-insult')
    @cooldown(7)
    ...
    @commands.command('worsethanhitler,worstthanhitler')
    @cooldown(5)
    ...
    @commands.command('wanted,chatroulette,frame,art')
    @cooldown(10)
    ...
    #
    # @commands.command('captchatext,captchatxt,generatecaptcha,gen-captcha,gencaptcha,capt')
    # @cooldown(10)
    # async def captcha(self, ctx, *args):
    #     await ctx.trigger_typing()
    #     capt = 'username601' if len(args) == 0 else ' '.join(args)
    #     return await ctx.bot.util.send_image_attachment(ctx,
    #                                                     'https://useless-api.vierofernando.repl.co/captcha?text={}'.format(
    #                                                         capt))

    @commands.command('baby,wolverine,disgusting,f,studying,starvstheforcesof')
    @cooldown(10)
    ...
    @commands.command('changedmymind')
    @cooldown(10)
    async def changemymind(self, ctx, *args):
        if len(args) == 0: return await ctx.bot.util.send_error_message(ctx, "Error! You need a text...")
        await ctx.trigger_typing()
        return await ctx.bot.util.send_image_attachment(ctx,
                                                        'https://nekobot.xyz/api/imagegen?type=changemymind&text=' + ctx.bot.util.encode_uri(
                                                            ' '.join(args)) + '&raw=1')

    @commands.command('gimme,memz,memey')
    @cooldown(5)
    ...
    async def floor(self, ctx, *args):
        if len(args) == 0:
            text = 'I forgot to put the arguments, oops'
        else:
            text = str(' '.join(args))
        ...
        if len(ctx.message.mentions) > 0:
            ...
            if len(args) > 2:
                ...
                text = 'I forgot to put the arguments, oops'
        return await ctx.bot.util.send_image_attachment(ctx,
                                                        'https://api.alexflipnote.dev/floor?image=' + auth + '&text=' + ctx.bot.util.encode_uri(
                                                            text[0:50]), alexflipnote=True)

    @commands.command('doctor,terrifying,terrified,eye-doctor,eyedoctor,scary,frightening')
    @cooldown(2)
    ...
    async def modern_meme(self, ctx, *args):
        ...
        def check(m):
            if ((m.channel != ctx.channel) or (m.author != ctx.author)):
                return False
            elif (m.content in keys) and (not m.content.isnumeric()):
                return True
        ...
        await ctx.send(
            embed=discord.Embed(title="Please provide your meme template from the available ones below. (in number)",
                                description="\n".join([
        ...
        if message is None: return await ctx.bot.util.send_error_message(ctx,
                                                                         "You did not respond in time. Meme-generation canceled.")
        link = self.meme_templates["bottom_image"][
            (keys[int(message.content) - 1] if message.content.isnumeric() else message.content)]
        format_text = await ctx.bot.utils.wait_for_message(ctx, message="Now send your text content to be in the meme.",
                                                           timeout=60.0)
        if format_text is None: return await ctx.bot.util.send_error_message(ctx,
                                                                             "You did not respond in time. Meme-generation canceled.")
        await ctx.trigger_typing()
    ...
    async def top_bottom_text_meme(self, ctx, *args):
        ...
        if message is None: return await ctx.bot.util.send_error_message(ctx,
                                                                         "You did not respond in time. Meme-generation canceled.")
        link = self.meme_templates["topbottom"][
            (keys[int(message.content) - 1] if message.content.isnumeric() else message.content)]
        format_text = await ctx.bot.utils.wait_for_message(ctx,
                                                           message="Now send your top text and bottom text. Splitted by either spaces, commas, semicolon, or |.",
                                                           timeout=60.0)
        if format_text is None: return await ctx.bot.util.send_error_message(ctx,
                                                                             "You did not respond in time. Meme-generation canceled.")
        text1, text2 = ctx.bot.Parser.split_content_to_two(format_text.content.split())
        url = link.replace("{TEXT1}", ctx.bot.util.encode_uri(text1)[0:64]).replace("{TEXT2}",
                                                                                    ctx.bot.util.encode_uri(text2)[
                                                                                    0:64])
        await ctx.trigger_typing()
        return await ctx.bot.util.send_image_attachment(ctx, url)

    async def custom_image_meme(self, ctx, *args):
        message = await ctx.bot.utils.wait_for_message(ctx,
                                                       message="Please send a **Image URL/Attachment**, or\nSend a **ping/user ID/name** to format as an **avatar.**\nOr send `mine` to use your avatar instead.",
                                                       timeout=60.0)
        if message is None:
            return await ctx.bot.util.send_error_message(ctx, "You did not input a text. Meme making canceled.")
        elif "mine" in message.content.lower():
            url = ctx.author.avatar_url_as(size=512, format="png")
        else:
            url = ctx.bot.Parser.parse_image(message, tuple(message.content.split()))
        text = await ctx.bot.utils.wait_for_message(ctx,
                                                    message="Send top text and bottom text. Splitted by a space, comma, semicolon, or |.",
                                                    timeout=60.0)
        if text is None: return await ctx.bot.util.send_error_message(ctx,
                                                                      "You did not input a text. Meme making canceled.")
        text1, text2 = ctx.bot.Parser.split_content_to_two(tuple(text.content.split()))
        await ctx.trigger_typing()
        return await ctx.bot.util.send_image_attachment(ctx,
                                                        "https://api.memegen.link/images/custom/{}/{}.png?background={}".format(
                                                            ctx.bot.util.encode_uri(text1)[0:64],
                                                            ctx.bot.util.encode_uri(text2)[0:64], url))

    @commands.command('memegen,meme-gen,gen-meme,generatememe,generate-meme,meme-editor,meme_editor,memeeditor')
    @cooldown(5)
    async def mememaker(self, ctx, *args):
        m = await ctx.send(embed=discord.Embed(title="Please select your meme format:",
                                               description="**[A] **Classic meme, Top text, bottom text, background image.\n**[B] **Modern meme, Top text, bottom image\n**[C] **Custom classic meme, with a custom background.",
                                               color=ctx.guild.me.roles[::-1][0].color))

        def check_chosen(m):
            return ((m.channel == ctx.channel) and (m.author == ctx.author) and (
                    len(m.content) == 1) and m.content.lower() in ['a', 'b'])

        message = await ctx.bot.utils.wait_for_message(ctx, message=None, timeout=60.0)
        if message is None:
            return await m.edit(content='',
                                embed=discord.Embed(title="Meme-making process canceled.", color=discord.Color.red()))
        elif message.content.lower() == 'a':
            res = await self.top_bottom_text_meme(ctx, *args)
        elif message.content.lower() == 'c':
            res = await self.custom_image_meme(ctx, *args)
        else:
            res = await self.modern_meme(ctx, *args)
